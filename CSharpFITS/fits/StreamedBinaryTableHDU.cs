namespace nom.tam.fits
{
	...
	public class StreamedBinaryTableHDU : BasicHDU
    {
        ...
        protected class Config
        {
            ...
            {
                return $"_haveNRows = {_haveNRows} _haveStrings = {_haveStrings} _seekable = {_seekable} _mode = {_mode}";
            }

            readonly bool _haveNRows;
            readonly bool _haveStrings;
            readonly bool _seekable;
            readonly StringWriteMode _mode;
        }
                    ...
                    if (maxColWidths[i] > -1)
                    {
                        ...
                        for (int j = 0; !found && ie.Current != null; ++j, ie.MoveNext())
                        {
                            if (("TFORM" + (i + 1)).Equals(((DictionaryEntry)ie.Current).Key))
                            {
                                ...
                                myHeader.
                                  InsertCard(new HeaderCard($"TFORM{(i + 1)}", $"1PA({maxColWidths[i]})",
                                  null), j);
                                found = true;
                            }
                        }
                    ...
                    for (int j = 0; !found && ie.Current != null; ++j, ie.MoveNext())
                    {
                        if (("TFORM" + (i + 1)).Equals(((DictionaryEntry)ie.Current).Key))
                        {
                            ...
                            else
                            {
                                myHeader.RemoveCard(j);
                                myHeader.
                                  InsertCard(new HeaderCard($"TFORM{(i + 1)}", $"{_stringTruncationLength}A",
                                  null), j);
                                found = true;
                            }
                        }
