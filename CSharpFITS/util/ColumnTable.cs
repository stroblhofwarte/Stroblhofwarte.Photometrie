    ...
    public class ColumnTable : DataTable
    {
        ...
        protected internal virtual void InitializePointers()
        {
            var nbyte =
                ...
                0; var nshort = 0; var nint = 0; var nlong = 0;
            var nfloat = 0; var ndouble = 0; var nchar = 0; var nboolean = 0;
        ...
        public void DeleteRows(int row, int length)
        {
            ...
            for (int col = 0; col < arrays.Length; col += 1)
            {
                ...
                Array.Copy((Array)arrays[col], 0, (Array)newArr, 0, row * sz);

                // Copy whatever comes after the deletion
                Array.Copy((Array)arrays[col], (row + length) * sz,
                            (Array)newArr, row * sz, (nrow - row - length) * sz);
                arrays[col] = newArr;
            }
            nrow -= length;
            InitializePointers();
